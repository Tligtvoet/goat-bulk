...
//    salesQuery.exec();
    ...
    ui->tableWidget->setHorizontalHeaderLabels(QString("Item;Price;Quantity;Total Sales").split(";"));
    int i = 0;
    while(inventoryQuery.next()) {
        ui->tableWidget->insertRow(i);
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(inventoryQuery.value(1).toString()));
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(inventoryQuery.value(2).toString()));
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(inventoryQuery.value(3).toString()));
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(inventoryQuery.value(4).toString()));
        i++;
    }
}
